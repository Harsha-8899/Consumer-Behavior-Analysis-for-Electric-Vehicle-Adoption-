...
st.text('This is the web app to explore the customer behaviour for the adoption of electric vehicles.')
...
if uploaded_file:
    ...
    # Example plot 1: Histogram of a selected numeric column
    ...
    # Example plot 3: Scatter plot of two selected numeric columns
    ...
    # Example plot 4: Bar plot of a selected categorical column
    if categorical_columns:
        selected_cat_column = st.selectbox('Select a categorical column for bar plot', categorical_columns)
        fig, ax = plt.subplots()
        df[selected_cat_column].value_counts().plot(kind='bar', ax=ax)
        ax.set_title(f'Bar Plot of {selected_cat_column}')
        ax.set_xlabel(selected_cat_column)
        ax.set_ylabel('Count')
        st.pyplot(fig)

    # Example plot 5: Box plot of a selected numeric column
    if numeric_columns:
    ...
    # Example plot 6: Line plot of a selected numeric column (if dataset contains time series data)
    ...
    # Example plot 8: Density plot of a selected numeric column
    if numeric_columns:
        selected_density_column = st.selectbox('Select a numeric column for density plot', numeric_columns, key='density')
        fig, ax = plt.subplots()
        sns.kdeplot(df[selected_density_column], ax=ax)
        ax.set_title(f'Density Plot of {selected_density_column}')
        ax.set_xlabel(selected_density_column)
        st.pyplot(fig)

    # Example plot 9: Pairplot of numeric columns
    if numeric_columns:
        selected_pairplot_columns = st.multiselect('Select numeric columns for pairplot', numeric_columns, default=numeric_columns)
        if selected_pairplot_columns:
            fig = sns.pairplot(df[selected_pairplot_columns])
            st.pyplot(fig)

    # Example plot 10: Violin plot of a numeric column grouped by a categorical column
    if numeric_columns and categorical_columns:
        selected_violin_numeric = st.selectbox('Select a numeric column for violin plot', numeric_columns, key='violin_numeric')
        selected_violin_categorical = st.selectbox('Select a categorical column for violin plot', categorical_columns, key='violin_categorical')
        fig, ax = plt.subplots()
        sns.violinplot(x=df[selected_violin_categorical], y=df[selected_violin_numeric], ax=ax)
        ax.set_title(f'Violin Plot of {selected_violin_numeric} grouped by {selected_violin_categorical}')
        ax.set_xlabel(selected_violin_categorical)
        ax.set_ylabel(selected_violin_numeric)
        st.pyplot(fig)
